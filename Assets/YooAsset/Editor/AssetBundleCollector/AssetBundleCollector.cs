...
namespace YooAsset.Editor
{
	...
	public class AssetBundleCollector
	{
			...
			if (AssetDatabase.IsValidFolder(CollectPath))
			{
				...
				foreach (string assetPath in findAssets)
				{
					if (IsValidateAsset(assetPath) && IsCollectAsset(assetPath))
					{
						...
						{
							var collectAssetInfo = CreateCollectAssetInfo(command, group, assetPath, isRawAsset, IsAssembly);
		...
		private CollectAssetInfo CreateCollectAssetInfo(CollectCommand command, AssetBundleCollectorGroup group, string assetPath, bool isRawAsset, bool isAssembly)
		{
			string address = GetAddress(group, assetPath);
			string bundleName = GetBundleName(group, assetPath);
			List<string> assetTags = GetAssetTags(group);
			CollectAssetInfo collectAssetInfo = new CollectAssetInfo(CollectorType, bundleName, address, assetPath, assetTags, isRawAsset, isAssembly);
