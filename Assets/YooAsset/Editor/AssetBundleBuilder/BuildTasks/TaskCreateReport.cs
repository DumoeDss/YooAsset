	...
	public class TaskCreateReport : IBuildTask
	{
		...
		private void CreateReportFile(BuildParametersContext buildParametersContext, BuildMapContext buildMapContext, ManifestContext manifestContext)
		{
            ...
            foreach (var item in manifestContext.Manifests)
            {
				...
				PackageManifest manifest = item.Value;
				...
				{
					var mainBundle = manifest.BundleList.Find(_ => _.BundleName == manifest.BundleNameList[packageAsset.BundleID]);
            ...
            BuildLogger.Log($"资源构建报告文件创建完成：");//($"资源构建报告文件创建完成：{filePath}");
		...
		private List<string> GetDependBundles(PackageManifest manifest, PackageAsset packageAsset)
		{
			...
			foreach (var dependBundleID in packageAsset.DependIDs)
			{
				string dependBundleName = manifest.DependBundleNameList[dependBundleID];
