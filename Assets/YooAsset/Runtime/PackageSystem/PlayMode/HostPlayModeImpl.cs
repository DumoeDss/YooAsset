...
namespace YooAsset
{
	internal class HostPlayModeImpl : IPlayModeServices, IBundleServices, IRemoteServices
	{
		...
		{
			string remoteMainURL = GetRemoteMainURL(packageBundle.FileName, packageBundle.PackageName);
			string remoteFallbackURL = GetRemoteFallbackURL(packageBundle.FileName, packageBundle.PackageName);
		...
		public bool IsBuildinPatchBundle(PackageBundle patchBundle)
		{
		...
		{
			return _queryServices.QueryStreamingAssets(packageBundle.PackageName, packageBundle.FileName);
		}
